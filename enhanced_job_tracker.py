...
class EnhancedJobApplicationTracker:
    ...
    def extract_company_from_email(self, email_address, email_content=""):
        ...
        # For complex domains with subdomains, be more careful about extraction
        domain_parts = domain.split('.')
        
        # If domain has multiple parts, look for the main company name
        if len(domain_parts) >= 2:
            # For domains like m.learn.coursera.org, alerts.t.redventures.com
            # Try to find the main company part
            potential_company = None
            
            # Look for recognizable company names in any part
            for part in domain_parts:
                if len(part) > 2:  # Skip single letters and very short parts
                    potential_company = part
                    break
            
            # If we didn't find a good part, use the full domain minus common TLDs
            if not potential_company or len(potential_company) <= 2:
                # Remove common TLDs and take the last meaningful part
                meaningful_parts = [part for part in domain_parts if part not in ['com', 'org', 'net', 'edu', 'gov']]
                if meaningful_parts:
                    potential_company = meaningful_parts[-1]  # Take the last meaningful part
                else:
                    potential_company = domain_parts[0]  # Fallback to first part
        else:
            potential_company = domain_parts[0]
        
        # Extract company name from domain
        company_name = potential_company.capitalize() if potential_company else "Unknown"
    ...
    def extract_hr_contact(self, sender_name, sender_email):
        """Extract HR contact name with improved email address handling"""
        if not sender_name or sender_name.strip() == "":
            # If no sender name, try to extract meaningful info from email
            if sender_email:
                return self.extract_contact_from_email(sender_email)
            return "LinkedIn"
        ...
        # If it's just an email or no-reply, try to extract from email
    ...
    def extract_contact_from_email(self, email_address):
        """Extract contact information from email address when name is not available"""
        if not email_address:
            return "LinkedIn"
        
        email_lower = email_address.lower()
        
        # If it's a noreply or automated email, try to get meaningful info
        if any(pattern in email_lower for pattern in ['noreply', 'no-reply', 'alerts', 'notifications']):
            # Extract domain info
            domain = email_address.split('@')[-1] if '@' in email_address else ""
            
            # For complex domains, include more context
            if domain:
                domain_parts = domain.split('.')
                
                # For domains like m.learn.coursera.org or alerts.t.redventures.com
                # Include enough context to be meaningful
                if len(domain_parts) >= 3:
                    # Take last 2-3 meaningful parts
                    meaningful_parts = [part for part in domain_parts if part not in ['com', 'org', 'net', 'www']]
            ...
            # For regular emails, try to extract name from local part
            local_part = email_address.split('@')[0] if '@' in email_address else email_address
            
            # If local part looks like a name (contains dots, letters), use it
            if '.' in local_part and not local_part.startswith('noreply'):
            ...
            # Otherwise include the full email for context
            return email_address
    
    def is_outbound_email(self, sender_email, account_email):
        """Check if this is an email I sent (outbound)"""
        if not sender_email or not account_email:
            return False
        
        # Check if sender email matches any of my configured accounts
        my_accounts = [account.lower() for account in ACCOUNTS]
        return sender_email.lower() in my_accounts
    
    def can_extract_meaningful_company(self, email_address):
        """Check if we can extract a meaningful company name from email address"""
        if not email_address:
            return False
        ...
        try:
            ...
            # Skip common email providers - these don't give meaningful company info
            common_providers = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 
                              'aol.com', 'icloud.com', 'live.com']
            ...
            # Check if we can extract a meaningful company name
            domain_parts = domain.split('.')
            meaningful_parts = [part for part in domain_parts 
                              if len(part) > 2 and part not in ['com', 'org', 'net', 'edu', 'gov', 'www']]
            
            # If we have at least one meaningful part, we can extract company info
        ...
        try:
            ...
            for header in headers:
                ...
                if name == 'subject':
                    ...
                    reply_to = value
                    # Extract email from reply-to if it has name format
                    email_match = re.search(r'<(.+?)>', value)
                    if email_match:
    ...
    def process_message(self, message_content, account_email):
        ...
        try:
            ...
            reply_to = message_content.get('reply_to', '')
            
            # Determine if this is an outbound email (I sent it)
            is_outbound_msg = self.is_outbound_email(sender_email, account_email)
            
            # For outbound emails, use reply-to address if available
            if is_outbound_msg and reply_to:
                # Check if we can extract meaningful company from reply-to
                if not self.can_extract_meaningful_company(reply_to):
                    print(f"  ⚠️  Skipping outbound email - cannot extract company from reply-to: {reply_to}")
                    return None
                
                # Use reply-to as the effective recipient
                effective_email = reply_to
                effective_sender = ""  # We don't have sender name for reply-to
            else:
                # Use original sender info
                effective_email = sender_email
                effective_sender = message_content.get('sender', '')
            
            # Skip excluded messages based on effective email
            if self.should_exclude_message(effective_email, subject):
                return None
            
            # Parse date
            date_str = message_content.get('date', '')
            try:
                msg_date = self.parse_email_date(date_str)
            except:
                msg_date = datetime.now()
            
            # Extract company info from effective email
            ...
            # Extract HR contact from effective sender
            ...
            subject_lower = subject.lower()
            is_outbound = is_outbound_msg or any(keyword in subject_lower for keyword in [
            ...
            return {
                ...
                'is_outbound_msg': is_outbound_msg,
    ...
    def create_application_key(self, app):
        ...
        # Add contact email to key to prevent same person appearing multiple times
        contact_email = app.get('company_email', '').lower().strip()
        
        return f"{company}::{position}::{contact_email}"
    ...
    def deduplicate_applications(self, applications):
        ...
        # Group applications by company + position + contact
        application_groups = {}
    ...
    def show_summary(self, applications, weekly_data):
        ...
        if pdf_apps:
            ...
            for app in sorted(pdf_apps, key=lambda x: x['date'], reverse=True)[:5]:
                pdf_date = app.get('pdf_date')
                date_str = pdf_date.strftime('%m/%d/%Y') if pdf_date else 'Unknown'
                print(f"  • {app['company_name']} - {app.get('pdf_job_title', 'Unknown')} ({date_str})")
